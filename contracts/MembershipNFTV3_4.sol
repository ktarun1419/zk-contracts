...
// File @openzeppelin/contracts-upgradeable/metatx/ERC2771ContextUpgradeable.sol@v4.9.3
...
// File @openzeppelin/contracts-upgradeable/token/common/ERC2981Upgradeable.sol@v4.9.3
...
// File @openzeppelin/contracts-upgradeable/token/ERC721/extensions/ERC721RoyaltyUpgradeable.sol@v4.9.3
...
// File contracts/blurio/IOperatorFilterRegistry.sol
...
// File contracts/blurio/OperatorFiltererUpgradeable.sol
...
// File contracts/blurio/DefaultOperatorFiltererUpgradeable.sol
...
// File contracts/MembershipNFTV3_4.sol
    ...
    DefaultOperatorFiltererUpgradeable
{
    ...
    // not currently in use, need to delete values before removing this storage
    ...
    // get max tokenId for user
    function getActiveTokenId(address owner) public view returns (uint256) {
        require(balanceOf(owner) > 0, "no tokens");

        uint256 tokenId;
        for(uint256 i = _tokenIds.current(); i > 0; i--) {
            if(_ownerOf(i) != owner) continue;
            tokenId = i;
            break;
        }

        require(tokenId > 0, "no tokens");

        return tokenId;
    }

    // get list of token id owned by the user
    function getTokenIdsForOwner(address owner) public view returns (uint256[] memory){
        require(balanceOf(owner) > 0, "no tokens");
        
        uint256 count = _tokenIds.current();
        uint256[] memory owned = new uint256[](count); // set array to maximum length
        ...
        // set length of array to actual length
        assembly {
    ...
    function getTokenDetailsBulk(uint256 _fromTokenId, uint256 _toTokenId) public view returns(TokenData[] memory) {
        require(_fromTokenId < _toTokenId);

        uint256 count = _tokenIds.current();
        uint256 toTokenId = _toTokenId < count ? _toTokenId : count;
        uint256 totalCount = toTokenId - _fromTokenId;

        TokenData[] memory tokenData = new TokenData[](totalCount + 1); // include fromTokenId and toTokenId

        for(uint256 i = 0; i <= totalCount; i++) {
            uint256 tokenId = i + _fromTokenId;
            tokenData[i].ownerAddress = _ownerOf(tokenId);
            tokenData[i].tokenId = tokenId;
            if(tokenData[i].ownerAddress != address(0)) {
                tokenData[i].tokenUri = tokenURI(tokenId);
            }
        }
